...
inline int netlib_udp_add_socket(netlib_socket_set set, udp_socket sock)
{
	return netlib_add_socket(set, (netlib_generic_socket)sock);
}
...
inline int netlib_udp_del_socket(netlib_socket_set set, udp_socket sock)
{
	return netlib_del_socket(set, (netlib_generic_socket)sock);
}
...
inline int _netlib_socket_ready(netlib_generic_socket sock)
{
...
inline void _netlib_write16(uint16_t value, void* areap)
{
...
inline void _netlib_write32(uint32_t value, void* areap)
{
	uint8_t *area = (uint8_t*)areap;
...
inline uint16_t _netlib_read16(void* areap)
{
	uint8_t *area = (uint8_t*)areap;
	return ((uint16_t)area[0]) << 8 | ((uint16_t)area[1]);
}

inline uint32_t _netlib_read32(const void *areap)
{
	const uint8_t *area = (const uint8_t*)areap;
	return ((uint32_t)area[0]) << 24 | ((uint32_t)area[1]) << 16 | ((uint32_t)area[2]) << 8 | ((uint32_t)area[3]);
}

inline uint32_t netlib_swap_BE32(uint32_t val)
{
	val = ((val << 8) & 0xFF00FF00) | ((val >> 8) & 0xFF00FF);
	return (val << 16) | (val >> 16);
}

inline uint16_t netlib_swap_BE16(uint16_t val)
{
	return (val >> 8) | (val << 8);
}
